...
/* Private variables ---------------------------------------------------------*/
USBD_DescriptorsTypeDef Device_Desc = {
...
__ALIGN_BEGIN uint8_t USBD_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END = {
  ...
  LOBYTE(USB_VID_ST),           /* idVendor */
  HIBYTE(USB_VID_ST),           /* idVendor */
  LOBYTE(USB_PID_CHIPDIP),           /* idVendor */
  HIBYTE(USB_PID_CHIPDIP),           /* idVendor */
...
uint8_t *USBD_AUDIO_DeviceDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  uint8_t *USB_IDs = GetUSB_IDs();
  
  for (uint8_t i = 0; i < 4; i++)
    USBD_DeviceDesc[USB_IDs_START_INDEX + i] = USB_IDs[i];
...
uint8_t *USBD_AUDIO_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  USBD_GetString(GetAudioDevName(), USBD_StrDesc, length);
  if (*length <= 2)
...
uint8_t *USBD_AUDIO_ManufacturerStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  USBD_GetString(GetManufactString(), USBD_StrDesc, length);
