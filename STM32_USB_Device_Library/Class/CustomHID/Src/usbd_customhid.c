...
USBD_ClassTypeDef  USBD_CUSTOM_HID =
{
  ...
  NULL, //USBD_CUSTOM_HID_GetDeviceQualifierDesc,
};
...
/**
  ...
  * @retval status
  */
//uint8_t SendBuf[USB_MESSAGE_LENGTH];
static uint8_t USBD_CUSTOM_HID_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  UNUSED(epnum);
  USBD_CUSTOM_HID_HandleTypeDef *hhid;
  ...
  USBDevice_RecPacket(hhid->Report_buf);
...
/*  
  ...
  SendBuf[1] = 0x55;
  Counter++;
  USBD_CUSTOM_HID_SendReport(&USBD_Device, SendBuf, USB_MESSAGE_LENGTH);
  USBD_CUSTOM_HID_ReceivePacket(&USBD_Device);
*/
